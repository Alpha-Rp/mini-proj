...
class ChronologicalArbitrageEngine:
    ...
    def get_next_pattern(self) -> Optional[str]:
        ...
        if not self.current_mode or not self.pattern_templates.get(self.current_mode):
            return None
            
        patterns = self.pattern_templates[self.current_mode]
        if not patterns or self.current_pattern_index >= len(patterns):
            # Reset index and generate new patterns if we've reached the end
            self.current_pattern_index = 0
            self.pattern_templates[self.current_mode] = self._generate_patterns(10, 
                complexity=1 if self.current_mode == 'easy' else 2 if self.current_mode == 'moderate' else 3)
            patterns = self.pattern_templates[self.current_mode]
